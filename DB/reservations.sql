...
create procedure isreserved(IN roomnum int, IN checkin DATE, in checkout DATE) -- sanity check before creating hotel/spa reservation
	...
	IF
		DAYS < 0 THEN set ERR := "CANNOT MAKE RESERVATION DATES ARE THE INVALID!";
	LEAVE ir;
	ELSE
		SELECT COUNT(*) FROM hotel_reservation r where (r.roomnum = roomnum AND (r.checkout > checkin));
	end if;
END//
delimiter ;
...
create procedure make_hotel_reservation(IN usrname VARCHAR(50), IN roomnum INT, IN checkin DATE, IN checkout DATE)
	...
	DECLARE reserved INT;
	SELECT c.cust_id into cust_id from customer c where c.usrname = usrname;
	call isreserved(roomnum, checkin, checkout);
	SELECT FOUND_ROWS() into reserved;
	if reserved > 0
	THEN set err := "ROOM IS RESERVED ON GIVEN DATE(S)";
	LEAVE mkr;
	ELSE INSERT INTO hotel_reservation VALUES (0, roomnum, cust_id, checkin, checkout);
	END IF;
END//
delimiter ;
